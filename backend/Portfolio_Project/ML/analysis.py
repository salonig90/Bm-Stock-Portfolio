...
def predict_stock_price(symbol):
    ...
    Returns: {
        ...
        "weekly_prediction": [{"date": str, "price": float}, ...],
        "prediction_plot": base64_str
    }
    """
    # 1. Fetch historical data (last 1 year)
...
def compare_stocks(symbol1, symbol2):
    ...
    plt.plot(df.index, (df[symbol1] / df[symbol1].iloc[0]) * 100, label=f"{symbol1} (Normalized)", color='blue')
    plt.plot(df.index, (df[symbol2] / df[symbol2].iloc[0]) * 100, label=f"{symbol2} (Normalized)", color='green')
    
    plt.title(f"{symbol1} vs {symbol2} Performance Comparison (1 Year)")
    plt.xlabel("Date")
    ...
    return {
        ...
        "recommendation": f"{symbol1} is likely to outperform {symbol2}" if outperform_prob > 0.5 else f"{symbol2} is likely to outperform {symbol1}",
...
def run_gold_silver_analysis():
    ...
    plt.title("Gold vs Silver: Linear Regression")
    plt.legend()
    ...
    return {
        ...
        "plots": {
            ...
            "gold_silver_regression": regression_plot
        }
    }
